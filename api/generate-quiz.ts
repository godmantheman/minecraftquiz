...
export default async function handler(req: any, res: any) {
  // We only handle POST requests representing quiz generation
  if (req.method !== 'POST') {
...
- Example Options:
  ...
  3) 이두 에메랄드판
  4) 네더 석판 구결
- Example Explanation: "정답은 훈민정음입니다! 세종대왕님께서는 일반 백성 유저들이 글을 몰라 크리퍼 피해를 입듯 불이익 당하는 것을 구호코자, 발음 기관을 본떠 고효율 훈민정음 포탈을 조합하셨습니다!"`;
...
You MUST return a JSON object matching this schema ALWAYS:
{
  ...
  try {
    ...
    if (!key) {
      res.status(500).json({ 
        error: '마인크래프트 원자로 동력이 차단되었습니다: GEMINI_API_KEY 환경 변수가 Vercel 설정에 정의되어 있지 않습니다. Vercel 대시보드(Settings > Environment Variables)에서 GEMINI_API_KEY 항목을 확인해 주세요.' 
      });
      return;
    }
    ...
    res.status(200).json(completeWorkbook);
  } catch (geminiError: any) {
    console.error('Gemini call failed on Vercel:', geminiError.message || geminiError);
    res.status(500).json({
